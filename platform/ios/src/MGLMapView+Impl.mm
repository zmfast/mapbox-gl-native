...
void MGLMapViewImpl::onStyleImageMissing(const std::string& imageIdentifier) {
    ...
    if ([mapView.delegate respondsToSelector:@selector(mapView:didFailToLoadImage:)]) {
        MGLImage *imageToLoad = [mapView.delegate mapView:mapView didFailToLoadImage:imageName];
        if (imageToLoad) {
            auto image = [imageToLoad mgl_styleImageWithIdentifier:imageName];
            mapView.mbglMap->getStyle().addImage(std::move(image));
        }
    }
}
